...
void BuildBoid(glm::vec3 tip_position);
...
std::vector<struct Boid> g_boids;
...
int main(int argc, char *argv[]) {
  ...
  GLFWwindow *window;
  window = glfwCreateWindow(800, 600, "INF01047 - 232022 - Bruno Bonatto", NULL,
                            NULL);
  if (!window) {
    glfwTerminate();
  ...
  // para renderização. Veja slides 176-196 do documento
  ...
  // TextureImage0 LoadTextureImage(
  //     "../../data/tc-earth_nightmap_citylights.gif"); // TextureImage1
  ...
  // Habilitamos o Backface Culling. Veja slides 23-34 do documento
  // Aula_13_Clipping_and_Culling.pdf.
  ...
  //                   glm::vec3(0.0f, 0.0f, 0.0f)};
  // struct Boid b3 = {3, 3, glm::vec3(0.0f, 0.0f, 1.0f),
  //                   glm::vec3(0.0f, 0.0f, 0.0f)};
  // struct Boid b4 = {4, 1, glm::vec3(0.0f, 0.0f, 0.0f),
  //                   glm::vec3(0.0f, 0.0f, 0.0f)};
  // struct Boid b5 = {5, 2, glm::vec3(1.0f, 1.0f, 1.0f),
  //                   glm::vec3(0.0f, 0.0f, 0.0f)};
  // g_boids.push_back(b1);
  // g_boids.push_back(b2);
  // g_boids.push_back(b3);
  // g_boids.push_back(b4);
  // g_boids.push_back(b5);
...
#define CONE 1
    ...
    std::map<int, glm::vec3> spd_per_flock;
    ...
    for (auto b : g_boids) {
      ...
      if (!spd_per_flock.count(b.flock_id)) {
        spd_per_flock[b.flock_id] = b.direction;
      } else {
        spd_per_flock[b.flock_id] += b.direction;
      }
    ...
    // 0.1f); glUniformMatrix4fv(model_uniform, 1, GL_FALSE,
    // glm::value_ptr(model)); glUniform1i(object_id_uniform, CONE);
    // DrawVirtualObject("cone");
    // model =
    //     Matrix_Translate(-1.0f, 0.0f, 0.0f) * Matrix_Scale(0.1f, 0.1f, 0.1f);
    // glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
    // glUniform1i(object_id_uniform, CONE);
    // DrawVirtualObject("cone");
    ...
    // TextRendering_ShowModelViewProjection(window, projection, view, model,
    // p_model);
...
// utilizados para renderização. Veja slides 176-196 do documento
// Aula_03_Rendering_Pipeline_Grafico.pdf.
//
void LoadShadersFromFiles() {
  ...
  model_uniform =
      glGetUniformLocation(program_id, "model"); // Variável da matriz "model"
  view_uniform = glGetUniformLocation(
      program_id, "view"); // Variável da matriz "view" em shader_vertex.glsl
  projection_uniform = glGetUniformLocation(
      program_id,
      "projection"); // Variável da matriz "projection" em shader_vertex.glsl
  object_id_uniform = glGetUniformLocation(
      program_id, "object_id"); // Variável "object_id" em shader_fragment.glsl
  object_type_uniform = glGetUniformLocation(program_id, "object_type");
